  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        ...
        >
          <div className="border-b border-surface-border px-3 py-2 text-xs font-semibold text-navy-700">
            Alerts &amp; notices
          </div>
          <div className="px-3 py-2 space-y-3 text-xs">
            {notices.length === 0 ? (
              <p className="text-navy-700/60">No notices for this session.</p>
            ) : (
              notices.map((n) => (
                <div
                  key={n.id}
                  className={`rounded-md border px-2 py-2 ${
                    n.kind === "welcome"
                      ? "border-accent/30 bg-accent-light/40 text-navy"
