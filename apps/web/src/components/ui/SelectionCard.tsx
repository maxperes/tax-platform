...
export function SelectionCard({ id, label, description, selected, onToggle }: Props) {
  return (
    <label
      htmlFor={id}
    ...
    >
      <input id={id} type="checkbox" checked={selected} onChange={onToggle} className="sr-only" />
      <span
        aria-hidden="true"
        className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border ${
          selected ? "border-accent bg-accent text-white" : "border-surface-border bg-white"
        }`}
      >
      ...
      <span>
        ...
        {description && (
          <span className="mt-0.5 block text-xs leading-relaxed text-navy-700/70">{description}</span>
        )}
      </span>
    </label>
  );
