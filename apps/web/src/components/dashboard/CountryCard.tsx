...
import type { CountryBlock } from "../../lib/interview/derive";

export function CountryCard({ block }: { block: CountryBlock }) {
  const rows = [
      ...
      <dl className="mt-4 space-y-2">
        {rows.map((row) => (
          <div key={row.label} className="flex items-baseline justify-between gap-4">
            ...
            <dd className="text-sm font-medium tabular-nums text-navy">{row.value}</dd>
          </div>
        ))}
      </dl>
