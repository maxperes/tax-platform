...
import { FINDING_LABELS, type Finding } from "../../lib/interview/derive";
import type { FindingStatus } from "../../lib/interview/types";
  ...
  return (
    <div className="flex flex-col gap-2 border-b border-surface-border py-4 last:border-0 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
      <div>
        ...
        <p className="mt-1 text-xs leading-relaxed text-navy-700/70">{finding.note}</p>
      </div>
      <div className="shrink-0">
        <StatusBadge tone={TONES[finding.status]}>{FINDING_LABELS[finding.status]}</StatusBadge>
      </div>
    </div>
  );
}
