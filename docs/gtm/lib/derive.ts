  ...
  return [
    {
      ...
      value: hasStays ? "Recorded from stay dates" : "Not answered",
      note: "Counted from entry and exit dates you recorded.",
    },
    {
...
export function residencyTimeline(record: DemoRecord): TimelineEvent[] {
  ...
  if (hasTrips) {
    events.push({
      ...
      detail: "Counted from stay dates you provided.",
    });
  }
...
export function countryBlocks(record: DemoRecord): CountryBlock[] {
  ...
  const foreignIncome = income.length;
  const foreignAssets = assets.filter(
    (asset) => asset.value !== "brazilian_companies",
  ).length;
  ...
  const hasUs = codes.has("us");
  const hasOther = Array.from(codes).some(
    (code) => code !== "us" && code !== "br",
  );
  ...
  return [
    {
      ...
      name: "United States",
      active: hasUs,
      incomeCount: hasUs ? foreignIncome : 0,
      assetCount: hasUs ? foreignAssets : 0,
      taxesPaid: hasUs ? taxesPaidLabel : "Not applicable",
      documentsAvailable: hasUs ? countAvailable(record, FOREIGN_DOCS) : 0,
      note: hasUs
        ? "Categories are shown here because you named the United States. This demo does not yet ask which country each item belongs to."
    ...
    },
    {
      ...
      name: "Brazil",
      active: true,
      incomeCount: assets.some((a) => a.value === "brazilian_companies") ? 1 : 0,
      assetCount: assets.filter((a) => a.value === "brazilian_companies").length,
      taxesPaid:
    ...
    },
    {
      ...
      active: hasOther,
      incomeCount: hasOther ? foreignIncome : 0,
      assetCount: hasOther ? foreignAssets : 0,
      taxesPaid: hasOther ? taxesPaidLabel : "Not applicable",
      documentsAvailable: hasOther ? countAvailable(record, FOREIGN_DOCS) : 0,
      note: hasOther
        ? "Based on the other countries you named. Country-by-country allocation comes later."
